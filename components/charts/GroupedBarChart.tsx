...
export function GroupedBarChart({ title, height = 400 }: GroupedBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    // CRITICAL: Check if prepared data actually has stacked keys (with '::' separator)
    // If Global data is mapped to regional geographies, data won't be stacked even if segments.length > 1
    const hasStackedKeys = [...allPreparedKeys].some(key => key.includes('::'))
    const isStacked = hasStackedKeys && (
    ...
    if (isStacked) {
      ...
      // Non-stacked: ALWAYS extract series from prepared data to ensure keys match
      // This is CRITICAL because prepareIntelligentMultiLevelData may create different keys
      // than what exists in the filtered records (e.g., Global mapped to regional geographies)
      series = extractSeriesFromPreparedData()

      console.log('📊 Non-stacked series extracted from prepared data:', series)
    }
