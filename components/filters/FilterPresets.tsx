...
export function FilterPresets() {
  ...
  const defaultPresets = useMemo<FilterPreset[]>(() => {
    console.log('🔄 Recalculating presets with data:', !!data, data ? 'Data exists' : 'No data')
    ...
    console.log('🔄 Preset filters calculated:', {
      topMarket: topMarketFilters,
      growthLeaders: growthLeadersFilters,
      emergingMarkets: emergingMarketsFilters
    })
  ...
  const applyPreset = (preset: FilterPreset) => {
    ...
    // Convert segments to advancedSegments format for the data processor
    const advancedSegments = preset.filters.segments?.map(segment => ({
      type: preset.filters.segmentType || filters.segmentType,
      segment: segment,
      id: `${preset.filters.segmentType || filters.segmentType}::${segment}`
    })) || []

    // Merge preset filters with current filters, preserving required fields
    const newFilters: any = {
      ...filters,  // Start with current filters to preserve all fields
      // Override with preset values
      geographies: preset.filters.geographies,
      segments: preset.filters.segments,
      segmentType: preset.filters.segmentType,
      yearRange: preset.filters.yearRange,
      dataType: preset.filters.dataType,
      viewMode: preset.filters.viewMode,
      // CRITICAL: Add advancedSegments for data processor to recognize selected segments
      advancedSegments: advancedSegments,
    }

    console.log('🎯 Final filters to apply:', newFilters)
    console.log('🎯 Geographies:', newFilters.geographies)
    console.log('🎯 Segments:', newFilters.segments)
    console.log('🎯 AdvancedSegments:', newFilters.advancedSegments)
    console.log('🎯 SegmentType:', newFilters.segmentType)
      ...
      {isExpanded && (
        <div className="grid grid-cols-2 gap-1 p-2 pt-0">
          ...
          <button
            ...
            onClick={() => {
              console.log('🖱️ Quick filter clicked:', preset.name)
              applyPreset(preset)
            }}
